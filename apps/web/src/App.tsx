...
import { LogOut, MoreHorizontal, Pencil, Plus, ShieldCheck, Trash2 } from 'lucide-react';
...
function RoomPage() {
  ...
  const openFile = async (fileId: string) => { try { const { url } = await api.viewFile(roomId, fileId); window.open(url, '_blank', 'noopener,noreferrer'); } catch (cause) { setError(cause instanceof Error ? cause.message : 'Unable to open file.'); } };
