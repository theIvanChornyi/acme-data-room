...
export function ShareDialog({ roomId, target, targetName, open, onClose }: { roomId: string; target: PublicShareTarget; targetName: string; open: boolean; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-30 grid place-items-center bg-slate-950/30 p-4" role="dialog" aria-modal="true" aria-labelledby="share-title">
    <section className="max-h-[calc(100vh-2rem)] w-full max-w-xl overflow-y-auto rounded-xl bg-white p-6 shadow-xl">
      ...
      <div className="mt-5 rounded-lg border bg-slate-50 p-4"><h3 className="text-sm font-semibold">People with access</h3><p className="mt-1 text-xs leading-5 text-slate-500">The recipient must sign in to Acme Data Room with Google once before you grant access.</p><div className="mt-3 flex gap-2"><input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="colleague@company.com" className="min-w-0 flex-1 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm outline-none placeholder:text-slate-400 focus:border-brand focus:ring-2 focus:ring-brand/15" /><button type="button" title="Grant read-only access" disabled={granting || !email.trim()} onClick={() => void grant()} className="inline-flex shrink-0 items-center gap-2 rounded-lg bg-brand px-3 py-2 text-sm font-medium text-white disabled:opacity-50">{granting ? <LoaderCircle className="animate-spin" size={16} /> : <UserPlus size={16} />}Grant access</button></div>{loading ? <p className="mt-3 text-sm text-slate-500">Loading people…</p> : userShares.length ? <ul className="mt-3 divide-y rounded-lg border bg-white">{userShares.map((share) => <li key={share.id} className="flex items-center justify-between gap-3 px-3 py-2.5"><div className="min-w-0"><p className="truncate text-sm font-medium text-slate-700">{share.email}</p><p className="text-xs text-slate-400">Viewer · added {new Date(share.createdAt).toLocaleDateString()}</p></div><button type="button" title={`Revoke access for ${share.email}`} aria-label={`Revoke access for ${share.email}`} onClick={() => void revokeUser(share.id)} className="rounded p-1.5 text-slate-500 hover:bg-red-50 hover:text-red-600"><Trash2 size={17} /></button></li>)}</ul> : <p className="mt-3 text-sm text-slate-500">No people have access yet.</p>}</div>
