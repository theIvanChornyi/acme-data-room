...
export function SharedRoom() {
  ...
  useEffect(() => {
    ...
    const request = isPermissionedShare ? api.sharedWithMeContents(accessKey, folderId) : api.publicContents(accessKey, folderId);
    void request.then(setContents).catch((cause) => setError(cause instanceof Error ? cause.message : 'This shared access is unavailable.'));
  }, [accessKey, folderId, isPermissionedShare]);
