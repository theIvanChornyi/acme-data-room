...
export function SharedRoom() {
  ...
  useEffect(() => {
    ...
    setContents(null);
    setLoading(true);
    const request = isPermissionedShare
      ? api.sharedWithMeContents(accessKey, folderId, activeCursor)
  ...
  const openFile = async (file: RoomItem) => {
    if (!accessKey) return;
    try {
      const { url } = isPermissionedShare
        ? await api.sharedWithMeViewFile(accessKey, file.id)
  ...
  const downloadFile = async (file: { id: string; name: string }) => {
    ...
    try {
      const { url } = isPermissionedShare
        ? await api.sharedWithMeDownloadFile(accessKey, file.id)
        : await api.publicDownloadFile(accessKey, file.id);
  ...
  return (
    <>
      <header className="border-b bg-white">
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between gap-2 px-5 font-semibold">
          <div className="flex items-center gap-2">
            <span className="rounded-lg bg-brand p-1.5 text-white">
              <ShieldCheck size={17} />
            </span>
            Acme Data Room
            <span className="ml-2 rounded-full bg-blue-50 px-2.5 py-1 text-xs font-medium text-brand">
              Read-only
            </span>
          </div>
          {isPermissionedShare && (
            <Link
              ...
              className="inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-sm font-medium text-slate-600 hover:bg-slate-100 hover:text-ink"
            >
              <ArrowLeft size={16} />
              Back to dashboard
            </Link>
      ...
      <main className="mx-auto max-w-6xl px-5 py-10">
        ...
        {contents.shareDescription && (
          <p className="mt-2 text-sm text-slate-600">{contents.shareDescription}</p>
        )}
        {contents.room.description &&
          contents.targetType === ShareTargetType.DATA_ROOM &&
          !contents.folder && (
            <p className="mt-2 text-sm text-slate-500">{contents.room.description}</p>
          )}
        <section className="mt-7 overflow-hidden rounded-xl border bg-white shadow-card">
          <div className="grid grid-cols-[minmax(0,1fr)_9rem_6rem_7rem] gap-4 border-b bg-slate-50 px-5 py-3 text-xs font-semibold uppercase tracking-wide text-slate-500">
            <span>Name</span>
            <span>Type</span>
            <span>Size</span>
            <span>Modified</span>
          ...
          {contents.items.length ? (
            contents.items.map((item) => (
              <button
                key={item.id}
                onClick={() =>
                  item.kind === 'folder' ? selectFolder(item.id) : void openFile(item)
                }
                className="grid w-full grid-cols-[minmax(0,1fr)_9rem_6rem_7rem] items-center gap-4 border-b px-5 py-3.5 text-left last:border-0 hover:bg-blue-50/50"
              >
            ...
            <div className="py-16 text-center text-sm text-slate-500">This folder is empty.</div>
          )}
        </section>
        <PageControls
          pageIndex={pageIndex}
