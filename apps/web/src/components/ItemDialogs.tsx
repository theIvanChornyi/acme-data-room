...
export function MoveFileDialog({ item, folders, onClose, onSubmit }: { item: RoomItem | null; folders: Array<{ id: string; name: string; depth: number }>; onClose: () => void; onSubmit: (folderId: string | null) => Promise<void> }) {
  ...
  if (!item) return null;
  const submit = async (event: FormEvent) => { event.preventDefault(); setSaving(true); try { await onSubmit(folderId || null); onClose(); } catch (cause) { setError(cause instanceof Error ? cause.message : 'Unable to move file.'); } finally { setSaving(false); } };
  return <Dialog title="Move file"><form onSubmit={submit}><p className="text-sm text-slate-500">Choose where to move <span className="font-medium text-ink">{item.name}</span>.</p><label className="mt-5 block text-sm font-medium">Destination<select value={folderId} onChange={(event) => setFolderId(event.target.value)} className="mt-2 w-full rounded-lg border bg-white px-3 py-2 outline-none focus:border-brand focus:ring-2 focus:ring-blue-100"><option value="">Data Room root</option>{folders.map((folder) => <option key={folder.id} value={folder.id}>{'— '.repeat(folder.depth)}{folder.name}</option>)}</select></label>{error && <p className="mt-2 text-sm text-red-600">{error}</p>}<DialogActions saving={saving} label="Move file" onClose={onClose} /></form></Dialog>;
}
...
function DialogActions({ saving, label, onClose }: { saving: boolean; label: string; onClose: () => void }) { return <div className="mt-6 flex justify-end gap-3"><button type="button" onClick={onClose} disabled={saving} className="px-3 py-2 text-sm font-medium">Cancel</button><button disabled={saving} className="rounded-lg bg-brand px-4 py-2 text-sm font-medium text-white disabled:opacity-50">{saving ? 'Saving…' : label}</button></div>; }
