...
export const api = {
  ...
  folders: (roomId: string, parentId?: string) => request<Array<{ id: string; name: string; parentId: string | null; depth: number; hasChildren: boolean }>>(`/data-rooms/${roomId}/folders${parentId ? `?parentId=${parentId}` : ''}`),
  folderOptions: (roomId: string) => request<Array<{ id: string; name: string; parentId: string | null; depth: number }>>(`/data-rooms/${roomId}/folder-options`),
