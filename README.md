...
- Checkbox selection in the workspace, with a streamed ZIP download for any mix of files and folders, plus bulk deletion with a complete impact warning.
...
- **Bulk workspace operations.** The browser sends only selected IDs (at most 100 of each kind); the API rechecks ownership, collapses nested selected folders to their roots, and ignores files already covered by a selected folder. ZIP creation reads the database in 100-file keyset pages and pipes one short-lived, authorized Storage stream at a time into the response. Browsers with the File System Access API stream this response directly to the selected file; a Blob download remains as the compatibility fallback. Empty folders are retained in the archive and duplicate archive paths receive a suffix. For exports that exceed the host's response-duration limit, the same iterator can be moved to an outbox worker that writes the ZIP to private object storage and returns a time-limited URL.
...
Bulk downloads use the same principle: a ZIP is streamed file-by-file from private Storage with keyset pages of 100 files, so the API never buffers the selection. Browsers that support the File System Access API stream directly to disk; the Blob fallback is retained for compatibility. The interactive request is appropriate for normal due-diligence exports; for exceptionally large archives, create a durable export job and persist the resulting archive in private object storage to avoid serverless response-duration limits.
